...
class Simple_Set_DETR(pl.LightningModule):
    """
    A naive implementation of Simple_Set_DETR.
    """
    def __init__(self, env_len=1, obj_len=2, out_set_size=4, hidden_dim=256, nheads=4,
    ...
    def forward(self, x, debug=False):
        ...
        h_objs_padded = pad(h_objs)                             # Shape: [BS, M, HIDDEN_DIM]
        ...
        # The transformer requires the input to have the format [S, N, E]
        # Where S is the input sequence length, N is the batch size, and E is the feature size
        # h_objs_padded = torch.transpose(h_objs_padded, 0, 1)    # Shape: [M, BS, HIDDEN_DIM]
        # h_objs = torch.transpose(h_objs, 0, 1)                  # Shape: [N, BS, HIDDEN_DIM]
        # h_env = torch.transpose(h_env, 0, 1)                    # Shape: [M, BS, HIDDEN_DIM]
        
        h = self.transformer(h_objs, h_objs_padded+h_env)
...
def train_pl():
    ...
    model = Simple_Set_DETR(1, 2, 4, hidden_dim=400)
...
def evaluate(model=None, path=None):
    ...
    for batch, (x, y) in enumerate(eval_data_loader):
        ...
        print(pred_matched)
        print("Pred Vel")
        print(pred['pred_pos'] - x[0, 1:None].reshape(-1,2))
        print()
