...
from obj_dqn import QNetwork, get_state, get_cont_state
# from examples.AC_lambda import ACNetwork
...
class agent:
    def __init__(self, env, network_params, which_agent, gui):
        self.in_channels = env.n_channels + 2
    ...
    def select_action(self):
        if (self.which_agent == 'DQN'):
            ...
            with torch.no_grad():
                action = self.network(self.s_cont).max(1)[1].view(1, 1)
        elif (self.which_agent == 'AC'):
            # Draw action from multinomial specified by policy network
            with torch.no_grad():
                action = torch.multinomial(self.network(self.s_cont)[0], 1)[0]
        else:
            raise ValueError('Unknown agent type')
        return action
    ...
    def initialize(self):
        ...
        self.env.reset()
        self.s_cont = get_cont_state(self.env.continuous_state())
    ...
    def play(self):
        ...
        self.G += reward
        s_cont_prime =get_cont_state(self.env.continuous_state())

        # Progress the game
        self.s_cont = s_cont_prime
